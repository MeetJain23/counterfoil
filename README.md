...
cannot be `allowed` while any clause blocks it — both enforced in `__post_init__`,
...
after tearing off the other half — the retained proof that the transaction
happened.
...
- Secrets are scanned at every commit by [tools/scan_secrets.py](tools/scan_secrets.py),
  which is itself covered by tests — a security control that is not tested is a
  claim, not a control
